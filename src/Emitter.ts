    ...
    private _createTreeNodes(docs: TAnyDoclet[])
    {
        ...
        for (let i = 0; i < docs.length; ++i)
        {
            ...
            if (!this._treeNodes[doclet.longname])
            {
                ...
                const parent = <IDocletTreeNode> this._treeNodes[doclet.longname];
                if ((doclet.kind === 'member') && (parent.doclet.kind === 'module')
                    && (doclet.meta) && (doclet.meta.code.name === "module.exports"))
                {
                    const exportDefault : IExportDefaultDoclet = {
                        kind: 'export default',
                        comment: doclet.comment,
                        meta: doclet.meta,
                        undocumented: doclet.undocumented,
                        name: doclet.name,
                        longname: doclet.longname,
                        scope: '' // not generated by jsdoc, but required by the IDocletBase declaration as defined.
                    };
                    debug(`Emitter._createTreeNodes(): converting ${docletDebugInfo(doclet)} into ${docletDebugInfo(exportDefault)}`);
                    parent.children.push({ doclet: exportDefault, children: [] });
                } else {
