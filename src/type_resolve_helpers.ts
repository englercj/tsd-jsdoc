...
class StringTreeNode {
    ...
    constructor(
        ...
        public parent: StringTreeNode | null,
    ) {}
...
function generateTree(name: string, parent: StringTreeNode | null = null): StringTreeNode | null {
    ...
    for (let i = 0; i < parts.length; ++i) {
        ...
        if (part.endsWith('.')) {
            ...
            if (matchingIndex === -1) {
                warn(
                    `Unable to find matching '<', '>' brackets in '${part}', defaulting to \`any\``,
                    name,
                );
                return anyNode;
            }

            const node = new StringTreeNode(
    ...
    switch (node.type) {
        case ENodeType.OBJECT:
            ...
            for (var i = 0; i < node.children.length; i = i + 2) {
                ...
                if (!valType) {
                    warn(
                        'Unable to resolve object value type, this is likely due to invalid JSDoc. Defaulting to `any`.',
                        node,
                    );
                    valType = anyTypeNode;
                }
        ...
        case ENodeType.GENERIC:
            ...
            if (upperName === 'OBJECT') {
                ...
                if (!keyType) {
                    warn(
                        `Unable to resolve object key type, this is likely due to invalid JSDoc. Defaulting to \`string\`.`,
                    );
                    keyType = strTypeNode;
                } else if (
                    node.children[0].type !== ENodeType.TYPE ||
                    (node.children[0].name !== 'string' && node.children[0].name !== 'number')
                ) {
                    warn(
                        `Invalid object key type. It must be \`string\` or \`number\`, but got: ${node.children[0].name}. Defaulting to \`string\`.`,
                    );
                    keyType = strTypeNode;
                }
...
export function toKeywordTypeKind(k: string): ts.KeywordTypeNode['kind'] | null {
    ...
    switch (k) {
        ...
        case 'BOOL':
            return ts.SyntaxKind.BooleanKeyword; // alias
        case 'STRING':
            return ts.SyntaxKind.StringKeyword;
        case 'SYMBOL':
            return ts.SyntaxKind.SymbolKeyword;
        case 'THIS':
            return ts.SyntaxKind.ThisKeyword;
        case 'VOID':
...
export function resolveVariable(
    doclet: IDocletProp,
): ts.Token<ts.SyntaxKind.DotDotDotToken> | undefined {
    if (doclet.variable) {
...
export function resolveHeritageClauses(
    doclet: IClassDoclet,
    onlyExtend: boolean,
): ts.HeritageClause[] {
...
export function resolveType(t: IDocletType, doclet?: TTypedDoclet): ts.TypeNode {
    if (!t || !t.names || t.names.length === 0) {
        ...
        if (doclet) {
            warn(
                `Unable to resolve type for ${doclet.longname ||
                    doclet.name}, none specified in JSDoc. Defaulting to \`any\`.`,
                doclet,
            );
        } else {
            warn(
                `Unable to resolve type for an unnamed item, this is likely due to invalid JSDoc.` +
...
export function resolveTypeName(name: string, doclet?: TTypedDoclet): ts.TypeNode {
    if (!name) {
        warn(
            'Unable to resolve type name, it is null, undefined, or empty. Defaulting to `any`.',
            doclet,
        );
        return anyTypeNode;
    }
...
export function createTypeLiteral(nodes: IPropDesc[]): ts.TypeLiteralNode {
    ...
    for (let i = 0; i < nodes.length; ++i) {
        ...
        const t = node.children.length
            ? createTypeLiteral(node.children)
            : resolveType(node.prop.type);
...
export function createFunctionParams(
    doclet: IFunctionDoclet | ITypedefDoclet | IClassDoclet,
