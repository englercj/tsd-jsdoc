...
function validateClassLikeChildren(
    children: ts.Node[] | undefined,
    validate: (n: ts.Node) => boolean,
    msg: string,
) {
    ...
    if (children) {
        for (let i = children.length - 1; i >= 0; --i) {
            ...
            if (!validate(child)) {
                warn(
                    `Encountered child that is not a ${msg}, this is likely due to invalid JSDoc.`,
                    child,
                );
                children.splice(i, 1);
            }
    ...
    if (children) {
        for (let i = children.length - 1; i >= 0; --i) {
            ...
            if (
                ...
                warn(
                    'Encountered child that is not a supported declaration, this is likely due to invalid JSDoc.',
                    child,
                );
                children.splice(i, 1);
            }
...
export function createNamespaceMember(doclet: IMemberDoclet): ts.VariableStatement {
    ...
    return handleComment(
        doclet,
        ts.createVariableStatement(mods, [
